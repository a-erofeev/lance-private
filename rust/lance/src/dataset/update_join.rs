...
use super::{UpdateJoinOptions, UpdateJoinStrategy, WriteParams, blob};
...
async fn run_external_update(
    ...
    while let Some(batch) = updater.next().await? {
        ...
        let (updated_batch, matched_rows) = patch_cursor
            ...
            .await?;
        for (field, column) in updated_batch
            .schema()
            .fields()
            .iter()
            .zip(updated_batch.columns())
        {
            HashJoiner::check_lance_support_null(field.name(), column, fragment.dataset())?;
        }
        blob_context
            .validate_updates(&updated_batch, &matched_rows)
            .await?;
