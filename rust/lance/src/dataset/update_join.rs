...
use arrow_array::{
    Array, RecordBatch, RecordBatchIterator, RecordBatchReader, UInt32Array, UInt64Array,
};
use arrow_row::{OwnedRow, RowConverter, SortField};
use arrow_schema::{DataType, FieldRef, Schema as ArrowSchema, SchemaRef, SortOptions};
...
use lance_arrow::json::{
    arrow_json_to_lance_json, convert_json_columns, has_json_fields, is_arrow_json_field,
};
use lance_arrow::memory::MemoryAccumulator;
use lance_arrow::{SchemaExt, interleave_batches};
use lance_core::datatypes::{OnMissing, OnTypeMismatch, Schema};
...
impl JsonConvertingReader {
    fn new(inner: Box<dyn RecordBatchReader + Send>) -> Self {
        ...
        let fields: Vec<FieldRef> = original_schema
            .fields()
            .iter()
            .map(|field| {
                if is_arrow_json_field(field) || has_json_fields(field) {
                    Arc::new(arrow_json_to_lance_json(field))
                } else {
                    Arc::clone(field)
                }
            })
...
impl Iterator for JsonConvertingReader {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.inner
            .next()
            .map(|batch| batch.and_then(|batch| convert_json_columns(&batch)))
    }
}
...
pub(super) async fn update_columns_with_options(
    ...
    let right_reader: Box<dyn RecordBatchReader + Send> = if original_right_schema
        .fields()
        .iter()
        .any(|field| is_arrow_json_field(field) || has_json_fields(field))
    {
        Box::new(JsonConvertingReader::new(right_reader))
    } else {
        right_reader
    };
...
async fn run_hash_update(
    ...
    while let Some(batch) = updater.next().await? {
        ...
        {
            for (row_index, is_matched) in matched_rows.into_iter().enumerate() {
...
async fn run_external_update(
    ...
    while let Some(batch) = updater.next().await? {
        let updated_batch = patch_cursor
...
impl SortedPatchCursor {
    ...
    async fn merge_batch(
        ...
        Ok(interleave_batches(&source_batches, &indices)?)
    }
...
mod tests {
    ...
    use arrow_array::{ArrayRef, Int32Array, Int64Array, ListArray, StringArray, record_batch};
    ...
    async fn test_external_update_non_monotonic_stable_row_ids() {
        ...
        metadata.row_id_meta = Some(RowIdMeta::Inline(write_row_ids(&sequence)));
