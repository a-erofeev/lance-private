...
impl IoQueue {
    ...
    fn on_bytes_consumed(&self, bytes: u64, priority: u128, num_admitted: usize) {
        let event = {
            let mut state = self.state.lock().unwrap();
            state.bytes_avail += bytes as i64;
            for _ in 0..num_admitted {
...
impl<F: FnOnce(Response) + Send> Drop for MutableBatch<F> {
    fn drop(&mut self) {
        ...
        let response = Response {
            ...
            reserved_bytes: if self.bypass_backpressure {
                0
            } else {
                self.reserved_bytes
            },
...
struct DataChunk {
    task_idx: usize,
    /// Bytes reserved by this task, or `None` if the task was cancelled before admission.
...
impl<F: FnOnce(Response) + Send> DataSink for MutableBatch<F> {
    ...
    fn deliver_data(&mut self, data: DataChunk) {
        if let Some(reservation_bytes) = data.reservation_bytes {
...
impl Drop for Response {
    fn drop(&mut self) {
        self.io_queue
            .on_bytes_consumed(self.reserved_bytes, self.priority, self.num_admitted);
    }
}
                ...
                when_done: Box::new(move |data, was_admitted| {
                    ...
                    }
                    {
                        ...
                        let chunk = DataChunk {
                            data,
                            task_idx,
                            reservation_bytes: was_admitted.then_some(num_bytes),
                        };
...
mod tests {
    ...
    #[test]
    fn batch_refunds_only_admitted_priority_reservations() {
        let response = Arc::new(Mutex::new(None));
        let response_clone = response.clone();
        let io_queue = Arc::new(IoQueue::new(2, 1024, IoStats::new()));
        let priority = 7;
        {
            let mut state = io_queue.state.lock().unwrap();
            // One reservation belongs to this batch and the other represents an
            // unrelated request with the same priority.
            state.priorities_in_flight.push(priority);
            state.priorities_in_flight.push(priority);
            state.bytes_avail -= 8;
        }
        ...
        batch.deliver_data(DataChunk {
            task_idx: 0,
            reservation_bytes: Some(4),
            data: Err(Error::io("injected failure".to_string())),
        });
        batch.deliver_data(DataChunk {
            task_idx: 1,
            reservation_bytes: None,
            data: Err(Error::io("cancelled before admission".to_string())),
        });
        drop(batch);

        let rsp = response.lock().unwrap().take().unwrap();
        assert_eq!(rsp.num_admitted, 1);
        assert_eq!(rsp.reserved_bytes, 4);
        drop(rsp);

        let state = io_queue.state.lock().unwrap();
        assert_eq!(state.bytes_avail, 1020);
        assert_eq!(state.priorities_in_flight.len(), 1);
        assert!(state.priorities_in_flight.contains(priority));
    }
    ...
    fn assert_io_error(error: &Error) {
        if matches!(error, Error::IO { .. }) {
            return;
        }
        let Error::Wrapped { error: source, .. } = error else {
            panic!("expected an I/O error, got {error:?}");
        };
        assert!(
            matches!(source.downcast_ref::<Error>(), Some(Error::IO { .. })),
            "expected the wrapped source to be an I/O error, got {source:?}"
        );
    }
    ...
    #[case::lite(true)]
    #[tokio::test]
    async fn short_read_returns_io_error(#[case] use_lite_scheduler: bool) {
        let scheduler = ScanScheduler::new(
            scheduler_test_object_store(),
            SchedulerConfig {
                io_buffer_size_bytes: 8,
                use_lite_scheduler: Some(use_lite_scheduler),
            },
        );
        let file_scheduler = scheduler.open_reader(Arc::new(ShortReader {
